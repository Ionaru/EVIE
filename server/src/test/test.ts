...
describe('Application', function () {
    ...
    //   // xit('should have a DB pool after start', async function () {
    //   //   await www.init().catch(console.error.bind(console));
    ...
    describe('configuration', function () {
        ...
        it('should be able read from the config file', async function () {
            config.getProperty('testValueDoesNotExist');
        });

        it('shouldn\'t be empty', async function () {
            expect(config.config).to.not.eql({});
        });
    });
});
