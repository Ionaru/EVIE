...
export async function init(): Promise<void> {
    ...
    const port = normalizePort(process.env.PORT || config.getProperty('backend_port') || 3000);
    express.app.set('port', port);

    // Create the HTTP server and give it the Express application for settings
    server = http.createServer(express.app as any);
    ...
    socketServer.on('connection', async (socket: ISessionSocket) => {
        ...
        await socket.handshake.session.save(() => undefined);
        sockets.push(socket);
